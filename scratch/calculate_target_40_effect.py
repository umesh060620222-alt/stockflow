...
def analyze_target_40():
    ...
    for i, day in enumerate(test_days):
        ...
        # We check if Nifty reached the +40 points target
        hit_target = False
        target_idx = None
    ...
    print("\n" + "="*80)
    print("📊 40-POINT TARGET ANALYSIS REPORT (PAST 90 DAYS)")
    print("="*80)
    print(f"Total Trades Analyzed:        {total_trades}")
    print(f"Wins (hit +40.0 target):     {total_wins}")
    print(f"Raw Win Rate (No Stop Loss): {raw_win_rate:.1f}%")
    print("-"*80)
    
    # Calculate performance for different stop losses with a +40 target
    for sl in [20, 30, 40, 50, 60]:
        # Wins that get stopped out:
        stopped_wins = len(df_res[(df_res['hit_target'] == True) & (df_res['drawdown'] > sl)])
        clean_wins = total_wins - stopped_wins
        
        # Losses (all missed days are stopped out at SL)
        missed_days = total_trades - total_wins
        
        # Total losses = stopped wins + missed days
        total_losses = stopped_wins + missed_days
        
        net_points = (clean_wins * 40.0) - (total_losses * sl)
