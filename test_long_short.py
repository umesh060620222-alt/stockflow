...
def simulate_long_short(candles, nifty_close, nifty_ema):
    # Calculate ATR(14)
    prev_close = None
    tr_history = []
    ...
    for i, c in enumerate(candles):
        ...
        # ------------------------------------
        # We only evaluate short if long was not triggered on this same bar
            ...
            if s_stage == 1:
                ...
                if low <= short_trigger_level:
                    if is_valid_time and is_valid_vol and is_nifty_bearish:
                        ...
                        # Short exits: SL is above entry, Target is below entry
                        sl = entry * (1 + actual_sl_pct)
                        target = entry * (1 - actual_target_pct)
                        ...
                        for idx_w, w in enumerate(candles[i+1:], start=i+1):
                            ...
                            # Halfway to target (price drops halfway)
    ...
    for d in dates:
        ...
        for sym in SYMS:
            try:
                ...
                trades = simulate_long_short(candles, nifty_close, nifty_ema)
