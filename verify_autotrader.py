...
def test_autotrader():
    ...
    assert abs(trade["spot_target"] - 24080.0) < 0.1, f"Expected target 24080.0, got {trade['spot_target']}"
    assert abs(trade["spot_sl"] - 24035.0) < 0.1, f"Expected SL 24035.0, got {trade['spot_sl']}"

    # Test 30% progress breakeven trail
    # 30% progress is entry + 0.3 * 30.0 = 24059.0
    print("Testing 30% progress trail stop trigger...")
    trader._manage_active_position(MockKite(), 24058.0)
    assert not trader.active_trade["reached_halfway"], "Should not trail stop at 24058."
    
    trader._manage_active_position(MockKite(), 24060.0)
    assert trader.active_trade["reached_halfway"], "Should trail stop at 24060."
    ...
    trader._manage_active_position(MockKite(), 24081.0)
