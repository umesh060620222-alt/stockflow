...
def main():
    ...
    print("\n" + "="*105)
    print(f"{'SYMBOL':<15}{'SIDE':<8}{'ENTRY@':<8}{'EXIT@':<8}{'HELD':>6}{'ENTRY PX':>12}{'EXIT PX':>12}{'RESULT':<12}{'PnL (INR)':>12}")
    print("-"*105)
    
    # Filter closed trades for cleaner output
    closed_trades = [t for t in all_trades if t["result"] in ("WIN", "LOSS", "BREAKEVEN")]
    # Sort closed trades by entry time
    closed_trades = sorted(closed_trades, key=lambda x: x["entry_time"])
    
    for t in closed_trades:
        sym_clean = t["sym"].replace(".NS", "")
        exit_str = f"{t['exit']:.2f}" if t["exit"] else "-"
        duration_str = f"{t['duration']}m"
        print(f"{sym_clean:<15}{t['side']:<8}{t['entry_time']:<8}{t['exit_time']:<8}{duration_str:>6}{t['entry']:>12.2f}{exit_str:>12}{t['result']:<12}{t['pnl']:>12,.0f}")
    print("="*105)
    ...
    print(f"SUMMARY FOR TODAY: Wins: {wins} | Losses: {losses} | Breakeven: {be} | Open: {opens}")
    print(f"TOTAL NET PROFIT today: {total_pnl:,.0f} INR")
    print("="*105)
