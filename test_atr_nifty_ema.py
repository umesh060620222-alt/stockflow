...
def simulate_atr_exits_with_nifty_ema(candles, nifty_close, nifty_ema):
    # Calculate ATR(14)
    prev_close = None
    tr_history = []
...
def run_test():
    print("Downloading 5 days of historical 1m data (including Nifty)...")
    ...
    for d in test_dates:
        # Pre-process Nifty EMA for this day
        ...
        # Compute 15-period EMA on 1-minute close prices
        ...
        for sym in SYMS:
            try:
                ...
                trades = simulate_atr_exits_with_nifty_ema(candles, nifty_close, nifty_ema)
        ...
        print("\n" + "="*95)
        print(f"SESSION SUMMARY FOR DATE: {d} (WITH NIFTY 15-EMA TREND FILTER)")
        print("="*95)
        win_rate = (wins / (wins + losses) * 100) if (wins + losses) > 0 else 0.0
        print(f"Wins: {wins} | Losses: {losses} | Open: {opens} | Win Rate: {win_rate:.1f}%")
